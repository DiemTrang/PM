...
public class TaskController {
	 ...
	 *         upload(@RequestHeader HttpHeaders header, @RequestParam("files")
	 *         MultipartFile[] files, @RequestParam("req") String req) { MultipleRsp
	 *         res = new MultipleRsp();
	 * 
	 *         try { ObjectMapper mapper = new ObjectMapper(); UploadReq o =
	 *         mapper.readValue(req, UploadReq.class);
	 * 
	 *         // Get data Integer id = o.getId();
	 * 
	 *         for (int i = 0; i < files.length; i++) { String originalName =
	 *         files[i].getOriginalFilename() + ""; String type =
	 *         files[i].getContentType(); float size = (float) files[i].getSize();
	 * 
	 *         // Get extension int t = originalName.lastIndexOf(".") + 1; String
	 *         extension = originalName.substring(t);
	 * 
	 *         // Create name String path = id + "/Attachment"; String uuId =
	 *         UUID.randomUUID().toString(); String name = uuId + "." + extension;
	 * 
	 *         // Upload file to S3 String url = ""; if (ZConfig._allowUpload) {
	 *         InputStream in = files[i].getInputStream(); url = ZFile.upload(in,
	 *         name, path); }
	 * 
	 * 
	 *         Task m = new Task(); m.setBoard(originalName);
	 * 
	 *         TaskService.upload(m); }
	 * 
	 *         // Load file from DB if (files.length > 0) { List<SortDto> sort = new
	 *         ArrayList<SortDto>(); sort.add(new SortDto("uploadedOn", "DESC"));
	 *         PagingReq pr; pr = new PagingReq(new TaskFilter(), sort, false);
	 * 
	 *         return search(pr); } } catch (Exception ex) {
	 *         res.setError(ex.getMessage()); }
	 * 
	 *         return new ResponseEntity<>(res, HttpStatus.OK); }
	 */
